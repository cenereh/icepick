...
bool filesystem::FsReadFile(const std::wstring Path, uint8_t** Buffer, uint32_t& Size)
{
    HANDLE FileHandle = IceCreateFile(Path.c_str(), GENERIC_READ, 0, nullptr, OPEN_EXISTING, 0, nullptr);
    ...
    *Buffer = (uint8_t*)IceVirtualAlloc(0, Size, MEM_COMMIT, PAGE_READWRITE);
...
bool filesystem::FsWriteFile(const std::wstring Path, uint8_t* Buffer, uint32_t Size)
{
    HANDLE FileHandle = IceCreateFile(Path.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, 0, nullptr);
    ...
    if (!IceWriteFile(FileHandle, Buffer, Size, nullptr, nullptr))
    {
        return false;
    }
